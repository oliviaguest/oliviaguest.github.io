...
<ul class="cv">
  ...
  {% for pub in pubs %}
    ...
    <a href="{% if pub.doi %}//dx.doi.org/{{ pub.doi }}{% else %}{{ pub.pdf }}{% endif %}">{{ pub.title }}</a>. <i>{{ pub.journal }}</i>. doi: <a href="//dx.doi.org/{{ pub.doi }}">{{ pub.doi }}</a>. <a href="{{ pub.pdf }}"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
    {% if pub.osf %}
